...
public record Student(UUID id, String studentNumber, String fullName, String email, UUID organizationUnitId,
                      StudentStatus status, long rowVersion, Instant createdAt, Instant updatedAt) {
    public Student {
        Objects.requireNonNull(id, "id");
        studentNumber = normalize(studentNumber, 32, "studentNumber");
    ...
    public static Student create(UUID id, String number, String name, String email, UUID unitId, StudentStatus status, Instant now) { return new Student(id, number, name, email, unitId, status, 0, now, now); }
